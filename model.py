...
def text_fgsm_attack(model, test_path, new_test_path) :
    attack_success = 0
    original_success = 0
    ...
    new_test_dataset = model._build_dataset(new_test_path)
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        batch2 = next(iter(new_test_dataloader))
        batch2["image"], batch2["text"], batch2["label"] = batch2["image"].to("cuda"), batch2["text"].to("cuda"), batch2["label"].to("cuda")
        ...
        # print(torch.eq(preds, perturbed_preds))
    ...
    return ((attack_success/len(test_dataset))*100)
...
def img_fgsm_attack(model, test_path, eps) :
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        torchvision.utils.save_image(batch_attack_images[0], '{}.png'.format(batch['id']))
        break
...
def fgsm_attack(model, test_path, new_test_path, eps) :
    attack_success = 0
    original_success = 0
    ...
    for batch in tqdm(test_dataloader, total=len(test_dataloader)):
        ...
        batch2 = next(iter(new_test_dataloader))
        batch2["text"] =  batch2["text"].to("cuda")

        perturbed_preds, _ = model.eval().to("cuda")(batch2["text"], batch_attack_images)
        perturbed_preds = perturbed_preds.max(1, keepdim=True)[1]
...
def adv_retrain(model, train_path, test_path, eps, hparams):

    img_mod, text_mod,  label_mod = [], [], []
    train_dataset = model._build_dataset(train_path)
    ...
    for batch in tqdm(train_dataloader, total=len(train_dataloader)):
        ...
        img_mod.append(np.concatenate((batch['image'].cpu().detach().numpy(), batch_attack_images.cpu().detach().numpy()), axis=0))
        text_mod.append(np.concatenate((batch['text'].cpu(), batch['text'].cpu()), axis=0))
        label_mod.append(np.concatenate((batch['label'].cpu(), batch['label'].cpu()), axis=0))

    print(img_mod.shape)
    adv_model = HatefulMemesModel(hparams=hparams)
    adv_model.fit(text_mod,img_mod,label_mod)
...
new_test_path = data_dir / "AE_test_unseen.jsonl"
adv_train_path = data_dir / "HS_train.jsonl"
adv_test_path = data_dir / "HS_test.jsonl"
...
if args.load:
    load_path = Path.cwd()/hparams.get("output_path")/"epoch=5-step=1134-v1.ckpt"
    ...
    # adv_model.fit()
    # adv_retrain(hateful_memes_model, train_path, test_path, eps, hparams)
    # print(text_fgsm_attack(hateful_memes_model, test_path, new_test_path))
    # print(img_fgsm_attack(hateful_memes_model, test_path, eps))
    # print(fgsm_attack(hateful_memes_model, test_path, new_test_path, eps))
    ...
    for csv_row in range(row):
        if(df.label[csv_row] == data.label[csv_row]):
            accuracy+=1
            comp.append(csv_row)
    print("Accuracy: %.2f%%"%((accuracy/len(data)) * 100) )
